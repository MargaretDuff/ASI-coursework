---
title: "ASI Coursework Report"
author: "02340, 02341, 02344"
output: pdf_document
fontsize: 9pt
...
\newcommand{\parmu}{\bm{\mu}}
\newcommand{\parcov}{\bm{\Sigma}}
\newcommand{\likelihood}{f(\vec{y}|\partheta)}
\newcommand{\posterior}{f(\partheta|\vec{y})}
...
We analyse the data of a drug trial on 50 litters of female rats. There were three rats per litter, one that recieved a drug treatment and two recieved a control. Let $T_i$ be the follow up time to tumor appearance in rat $i$, which we assume are independent and identically distributed (iid) Weibull random variables.
```{r include=FALSE}
...
We use \texttt{optim} with the BFGS method to compute the maximum likelihood estimate of the parameter $\theta^\top=(\beta_0,\beta_1,\log(\sigma))$ as well as the asymptotic standard error of each parameter estimate.
```{r}
...
```{r}
se_weib <- diag(solve(fit_weibull$hessian))^.5 #standard deviations
...
We now assume that $T_i\sim \mbox{log-logistic}(\mbox{shape}=1/\sigma,\mbox{scale}=\exp(\eta_i))$, and repeat the above analysis under this new model.
```{r}
...
theta0 <- c(1,1,1) #initial point
fit_llogis <- optim(par=theta0, fn=nll_llogis, method="BFGS", hessian=TRUE, rx=rx, times=times, status=status)
fit_llogis$convergence #check convergence
mle_llogis <- fit_llogis$par; mle_llogis #the optimal values
-fit_llogis$value #loglikelihood value at the mle
se_llogis <- diag(solve(fit_llogis$hessian))^.5 #standard errors
round(c(mle_llogis[2]-1.96*se_llogis[2], mle_llogis[2]+1.96*se_llogis[2]), 4)#confidence interval for beta1
```
...
Since $\vec{b} \sim N(\vec{0}, \sigma_b^2\vec{I})$, and using the Normal pdf, $\log\mleft[f(\vec{b}|\partheta)\mright] = -\sum_{j=1}^{n_b}\mleft\{\log(\sigma_b) + \log(2\pi)/2 + b_j^2/2\sigma_b^2 \mright\}$. The R function ``lfyb`` evaluates the joint log-density $\jointf = \jointf + \log\mleft[f(\vec{b}|\partheta)\mright]$:
```{r}
...
We now move to a Bayesian setting; assume the same model as above, but that $\theta=(\beta_0, \beta_1, \log(\sigma), \log(\sigma_b))^T$ is a random vector. We assume that $\sigma_b$ follows an exponential prior with rate 5 and is independent of the other parameters, which each follow improper uniform priors. This gives us the following log posterior function:
...
log.post <- function(beta0, beta1, log_sigma,log_sigma_b,b_tumour, b_dead,  tumour, dead) { 
  ...
  log.lik<-sum(log (dweibull(tumour$time, 1/exp(log_sigma), exp(beta0+beta1*tumour$rx+b_tumour))))+
    sum(log (pweibull(dead$time, 1/exp(log_sigma), exp(beta0+beta1*dead$rx+b_dead), lower.tail=FALSE)))# Now the log likelihood
  return(log.lik+log_pi0)# now the log posterior = log likelihood +log prior 
}
```
We use a normal centered on the current values as the proposal for $\partheta$, and each element of $b$ I will also use a symmetric multivariate normal distribution centered on the current values for the values of $b$.  We will need to tune the proposal standard deviations to get appropriate acceptance rates, aiming for about 25%.
...
proposed= rmvnorm(1, mean=c(current_beta0, current_beta1, current_log_sigma), 
                  lambda*covariance) #update remaining paramaters
...
```{r}
mh=MH2(5.0188886, -0.2376741, -1.3687252,-1.5976068, b_tumour0 =  rep(0, length(tumour$rx)),
...
plot(mh$beta0[show.plot],type="l",ylab=expression(beta[0]))
plot(mh$beta1[show.plot],type="l",ylab=expression(beta[1]))
plot(mh$log_sigma[show.plot],type="l",ylab=expression(log(sigma)))
plot(mh$log_sigma_b[show.plot],type="l",ylab=expression(log(sigma[b])))
```
...
acf(mh$beta0[-burn.in],xlab=expression(beta[0]))
acf(mh$beta1[-burn.in],xlab=expression(beta[1]))
acf(mh$log_sigma[-burn.in],xlab=expression(log(sigma)))
acf(mh$log_sigma_b[-burn.in],xlab=expression(log(sigma[b])))
```
...
par(mfrow=c(2,3),mar=c(4,4,1,1))
samp<-sample((1:nsteps)[-(burn.in)],nsteps/2)# For visualization purposes we take a random sample of the iterations retained (after discarding burn-in)
plot(mh$beta0[samp],mh$beta1[samp],xlab=expression(beta_0),ylab=expression(beta[1]),pch=".",cex=0.1)
plot(mh$beta0[samp],mh$log_sigma[samp],xlab=expression(beta_0),ylab=expression(log(sigma)),pch=".",cex=0.1)
plot(mh$beta0[samp],mh$log_sigma_b[samp],xlab=expression(beta_0),ylab=expression(log(sigma[b])),pch=".",cex=0.1)
plot(mh$beta1[samp],mh$log_sigma[samp],xlab=expression(beta_1),ylab=expression(log(sigma)),pch=".",cex=0.1)
plot(mh$beta1[samp],mh$log_sigma_b[samp],xlab=expression(beta_1),ylab=expression(log(sigma[b])),pch=".",cex=0.1)
plot(mh$log_sigma[samp],mh$log_sigma_b[samp],xlab=expression(log(sigma)),ylab=expression(log(sigma[b])),pch=".",cex=0.1)
```
...
```{r echo=FALSE, fig.height=3, fig.align='center'}
par(mfrow=c(2,2),mar=c(4,4,1,1))
hist(mh$beta0[-burn.in], freq = FALSE, breaks = 100, main=NULL, xlab=expression(beta[0]))
hist(mh$beta1[-burn.in], freq = FALSE, breaks = 100, main=NULL, xlab=expression(beta[1]))
hist(mh$log_sigma[-burn.in], freq = FALSE, breaks = 100, main=NULL, xlab=expression(log(sigma)))
hist(mh$log_sigma_b[-burn.in], freq = FALSE, breaks = 100, main=NULL, xlab=expression(log(sigma[b])))
```
...
Assume throughout that $N_i \sim \mathrm{Weibull}\mleft(\mathrm{shape}=1/\sigma, \mathrm{scale} = \alpha(s_i - \gamma)^{\delta}\mright)$, where $s_i > gamma$, and $\gamma$ denotes some minimum stress threshold below which the material will never fail. We use \texttt{optim} to find the maximum likelihood estimate of $\bm{\theta}=(\log(\alpha),\delta,\log(\sigma))^\top$ for an arbitary value of $\gamma$. We compute asymptotic $95\%$ confidence intervals for each of the unknown parameters and investigate the sensitivity of the results to the chosen value of $\gamma$. 
```{r}
...
gamma_sensitivity <- function(theta0, gam_list){#gam_list: gamma values to try, theta0: initial point for optim
  ...
  expgam <- expression(gamma)
  exps <- c(expression(log(alpha)), expression(delta), expression(log(sigma)))
  par(mfrow=c(2,3))#sensitivity plots
   plot(x <- gam_sensitivity_data$gamma, y <- gam_sensitivity_data$log_like, type = 'line', xlab = expgam, ylab = "log-like")
  plot(x <- gam_sensitivity_data$gamma, y <- gam_sensitivity_data$logalpha, type = 'line', xlab = expgam, ylab = exps[1]); plot(x <- gam_sensitivity_data$gamma, y <- gam_sensitivity_data$delta, type = 'line', xlab = expgam, ylab = exps[2]); plot(x <- gam_sensitivity_data$gamma, y <- gam_sensitivity_data$logsigma, type = 'line', xlab = expgam, ylab = exps[3]); plot(x <- gam_sensitivity_data$gamma, y <- gam_sensitivity_data$scale_param, type = 'line', xlab = expgam, ylab = "mean scale param")
...
We now estimate the full vector of unknown parameters - that is, we include $\gamma$ in our unknown parameter vector $\theta$. By definition of $\gamma$ (as the fatigue limit), we impose the constraint $0<\gamma<\min(s[!ro])=\min(s)$. Let $\theta_4 = \textrm{logit}\left(\frac{\gamma}{\min(s)}\right)$, i.e. we map $\gamma / \min(s)$ from (0,1) to $\mathbb{R}$, to make the optimisation work.
```{r}
...
```
The 95% confidence intervals for $\log(\alpha)$, $\delta$, $\log(\sigma)$ and logit($\gamma / \min(s)$) are given below. 
```{r echo=FALSE}
se_weibull2_gam <- diag(solve(fit_weibull2_gam$hessian))^.5 #standard errors
ci <- matrix(nrow=4, ncol=2); colnames(ci) <- c("lower", "upper")
rownames(ci) <- c("log alpha", "delta", "log sigma", "transformed gamma")
for(i in 1:4) {
  ci[i,] <- round(mle_gam[i] + 1.96*se_weibull2_gam[i]*c(-1,1), 4)
}; ci
```
We now investigate the lower 10\%  quantile of $N$ which in this case is given by 
...
points(x <- s, y <- N)
lines(x <- s, y <- weibull2_quantiles50, col = 'red', lty=2)
legend(110, 200000, legend=c("10% quantile", "median"),
       col=c("blue","red"), lty=1:2, cex=0.8)
```

Consider now the following Bayesian random-effects model that allows the fatigue limit to be different for each coupon. This is achieved by modelling the fatigue limit as an unobserved random variable $\Gamma_i$, with its own Weibull distribution. We must now also make the distribution of $N_i$ conditional on $\Gamma_i = \gamma_i$, but this remains the Weibull distribution considered above. We also assume that $\partheta$ is now a random vector; $\sigma_b$ follows an exponential prior with rate 5, independently of the other parameters which all have improper uniform priors. 
...
As in the previous section, we constrct a Metropolis-Hastings algorithm to sample from the posterior distribution (of $\partheta | \vec{y})$ and the random effects $\vec{b}$. The below computes a log posterior function for the model given parameters: $\log(\alpha), \delta, \log(\sigma), \mu_\gamma, \log(\sigma_\gamma)$ and $\gamma$ 
```{r}
...
plot(mh$log_alpha[show.plot],type="l",ylab=expression(log(alpha)))
plot(mh$delta[show.plot],type="l",ylab=expression(delta))
plot(mh$log_sigma[show.plot],type="l",ylab=expression(log(sigma)))
plot(mh$mu_gamma[show.plot],type="l",ylab=expression(mu[gamma]))
plot(mh$log_sigma_gamma[show.plot],type="l",ylab=expression(log(sigma[gamma])))
acf(mh$log_alpha[-burn.in],xlab=expression(log(alpha)))
acf(mh$delta[-burn.in],xlab=expression(delta))
acf(mh$log_sigma[-burn.in],xlab=expression(log(sigma)))
acf(mh$mu_gamma[-burn.in],xlab=expression(mu[gamma]))
acf(mh$log_sigma_gamma[-burn.in],xlab=expression(log(sigma[gamma])))
```
...
plot(mh$log_alpha[samp],mh$log_sigma_gamma[samp],xlab=expression(log(alpha)),ylab=expression(log(sigma[gamma])),pch=".",cex=0.1)
plot(mh$delta[samp],mh$log_sigma[samp],xlab=expression(delta),ylab=expression(log(sigma)),pch=".",cex=0.1)
plot(mh$delta[samp],mh$log_sigma_gamma[samp],xlab=expression(delta),ylab=expression(log(sigma[gamma])),pch=".",cex=0.1)
plot(mh$log_sigma[samp],mh$log_sigma_gamma[samp],xlab=expression(log(sigma)),ylab=expression(log(sigma[gamma])),pch=".",cex=0.1)
plot(mh$mu_gamma[samp],mh$log_alpha[samp],xlab=expression(mu[gamma]),ylab=expression(log(alpha)),pch=".",cex=0.1)
plot(mh$mu_gamma[samp],mh$delta[samp],xlab=expression(mu[gamma]),ylab=expression(delta),pch=".",cex=0.1)
plot(mh$mu_gamma[samp],mh$log_sigma[samp],xlab=expression(mu[gamma]),ylab=expression(log(sigma)),pch=".",cex=0.1)
plot(mh$mu_gamma[samp],mh$log_sigma_gamma[samp],xlab=expression(mu[gamma]),ylab=expression(log(sigma[gamma])),pch=".",cex=0.1)
```
...
Given some realisation of the random variable $\partheta$, the marginal distribution of $N_i$ may be recovered from the conditional distribution $N_i|\Gamma_i$ by integrating out $\Gamma_i$. This means that the likelihood of the data, $\likelihood$, is given by: $$
\likelihood = \prod_{i=1}^n f(n_i|\partheta) = \prod_{i=1}^n \mleft[\int_0^{s_i} f(n_i|\gamma_i, \partheta)f(\gamma_i|\partheta)\mathrm{d}\gamma_i\mright].
$$ Code for evaluating this likelihood is given below. In particular, the function ``integrand`` evaluates the integrand for a given $(n_i, \gamma_i, \partheta)$; ``marginal`` is a vector-valued function whose $i$th component is $f(n_i|\partheta)$, calculated using ``integrate``; and ``likelihood`` calculates $\likelihood$ using the product given above. A function which evaluates the prior distribution $f(\partheta)$ is also supplied.
...
To resolve this, we take our full sample from the posterior using a modified Metropolis-Hastings algorithm; specifically, let $\hat{\parmu}$ and $\hat{\vec{\Sigma}}$ denote the estimated mean vector and covariance matrix, respectively, of $\partheta|\vec{y}$ (calculated using the pilot chain simulated above). Then, for the final sample from the posterior, we use the $N(\hat{\parmu}, \hat{\vec{\Sigma}})$ as the proposal distribution. 
```{r}
set.seed(20000)
...
idx <- seq(from=1, to=nsamples/2, by=20)
for(i in 1:ncol(samples)) {
  ks <- ks.test(samples[idx, i], samples[nsamples/2 + idx, i])
  print(paste("p-value for", names[i], "=", ks$p.value))
}
```
...
par(mfrow=c(3,2), mar=c(3,4,3,4))
samples2 <- samples
for(i in c(1,3,5)) {
  samples2[,i] <- exp(samples2[,i])
}
names2 <- c(expression(alpha), expression(delta), expression(sigma), expression(mu[gamma]), expression(sigma[gamma]))
for(i in 1:5) {
  hist(samples2[,i], xlab=names2[i], ylab="Density", main=NULL, freq=FALSE, breaks=100)
}
```

The shape of the histogram for $\alpha$ is most striking, as it is highly positively skewed. The histograms for the the other parameters also clearly exhibit some features which suggest that $\partheta|\vec{y}$ does not follow a multivariate-Normal; all of them display varying degrees of asymmetry and skewness, for example. 

Finally, 95% credible intervals for each of the parameters are given below. 
```{r echo=FALSE}
ci <- matrix(nrow=5, ncol=2); colnames(ci) <- c("2.5%", "97.5%")
rownames(ci) <- c("alpha", "delta", "sigma", "mu_gamma", "sigma_gamma")
for(i in 1:ncol(samples)) {
  ci[i,] <- round(quantile(samples2[,i], probs=c(0.025, 0.975)), 3)
}; ci
```
