...
k1=ks.test(mh$beta0[-1000:-500], mh$beta0[-500]); k1$p.value
k2=ks.test(mh$beta1[-1000:-500], mh$beta1[-500]);k2$p.value
k3=ks.test(mh$log_sigma_b[-1000:-500], mh$log_sigma_b[-500]);k3$p.value
k4=ks.test(mh$log_sigma[-1000:-500], mh$log_sigma[-500]);k4$p.value
```
...
Again, we investigate the correlation between the parameters. 
```{r}
par(mfrow=c(3,2),mar=c(4,4,1,1))
samp<-sample((1:nsteps)[-(burn.in)],nsteps/2)# For visualization purposes we take a random sample of the iterations retained (after discarding burn-in)
plot(mh$beta0[samp],mh$beta1[samp],xlab=expression(beta_0),ylab=expression(beta_1),pch=".",cex=0.1)
...
```{r}
quantile(mh$beta1[-(burn.in)], c(.025, 0.975))#95% condfidence interval or beta_1
```
